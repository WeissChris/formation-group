...
export async function upsertProposal(proposal: DesignProposal): Promise<void> {
  ...
  if (isSupabaseConfigured() && supabase) {
    ...
    await safeUpsert('fg_proposals', {
      ...
      email_greeting_name: fresh.emailGreetingName ?? null,
      revisions_included: fresh.revisionsIncluded ?? null,
      revisions_note: fresh.revisionsNote ?? null,
      sample_ids: fresh.sampleIds ?? null,
      samples_blurb: fresh.samplesBlurb ?? null,
...
export async function upsertEstimate(estimate: Estimate): Promise<void> {
  ...
  if (isSupabaseConfigured() && supabase) {
    ...
    await safeUpsert('fg_estimates', {
      ...
      version_group_id: fresh.versionGroupId ?? null,
      status: fresh.status,
      ...
      category_complete: fresh.categoryComplete || {},
      category_kind: fresh.categoryKind || {},
      ...
      variation_labour_hours: fresh.variationLabourHours ?? null,
      variation_materials_cost: fresh.variationMaterialsCost ?? null,
...
function mapProposal(row: Record<string, unknown>): DesignProposal {
  return {
    ...
    emailGreetingName: (row.email_greeting_name as string | null) || undefined,
...
export function mapEstimate(row: Record<string, unknown>): Estimate {
  return {
    ...
    versionGroupId: (row.version_group_id as string | null) || undefined,
    ...
    categoryComplete: (row.category_complete as Record<string, boolean>) || {},
    ...
    variationLabourHours: row.variation_labour_hours != null ? Number(row.variation_labour_hours) : undefined,
    variationMaterialsCost: row.variation_materials_cost != null ? Number(row.variation_materials_cost) : undefined,
