...
export interface InvoiceEmailInput {
  ...
  description?: string
  projectAddress?: string
  lines: { description: string; amount: number }[]   // included claim lines, ex GST
  subtotalEx: number
  gst: number
  total: number
...
export function buildInvoiceEmailHtml(input: InvoiceEmailInput): string {
  const GREEN = '#3D5A3A', INK = '#1a1a1a', BODY = '#2d2d2d', MUTED = '#8A8580'
  const font = `-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif`
  const name = escapeHtml(clientGreetingNames(input.clientName))
  const address = input.projectAddress ? escapeHtml(input.projectAddress.trim()) : ''
  const desc = input.description ? escapeHtml(input.description.trim()) : ''
  const rows = input.lines.map(l => `
    <tr>
      <td style="padding:7px 0;font-size:13px;color:${BODY};border-bottom:1px solid #f0ede9;">${escapeHtml(l.description)}</td>
      <td align="right" style="padding:7px 0;font-size:13px;color:${INK};border-bottom:1px solid #f0ede9;white-space:nowrap;">${moneyCents(l.amount)}</td>
    </tr>`).join('')
  const totalRow = (label: string, value: string, strong = false) => `
    <tr>
      <td style="padding:5px 0;font-size:${strong ? 14 : 12}px;color:${strong ? INK : MUTED};${strong ? 'font-weight:600;' : ''}">${label}</td>
      <td align="right" style="padding:5px 0;font-size:${strong ? 15 : 12}px;color:${INK};${strong ? 'font-weight:600;' : ''}white-space:nowrap;">${value}</td>
    </tr>`
  ...
  return `<!doctype html><html><body style="margin:0;padding:0;background:#eceae7;font-family:${font};color:${BODY};">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#eceae7;"><tr><td align="center" style="padding:28px 12px;">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:100%;max-width:600px;background:#fff;border:1px solid #e7e4df;">
        <tr><td style="padding:34px 44px 0 44px;">
          <p style="margin:0 0 8px 0;font-size:12px;letter-spacing:0.2em;text-transform:uppercase;color:${GREEN};font-weight:600;">Formation Landscapes</p>
          <h1 style="margin:0;font-size:24px;font-weight:300;line-height:1.25;color:${INK};">Invoice ${escapeHtml(input.invoiceNumber)}</h1>
          ${desc ? `<p style="margin:8px 0 0 0;font-size:14px;font-weight:300;color:${BODY};">${desc}</p>` : ''}
          ${address ? `<p style="margin:6px 0 0 0;font-size:13px;font-weight:300;color:${MUTED};">${address}</p>` : ''}
        </td></tr>
        <tr><td style="padding:22px 44px 0 44px;">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 22px 0;">
            ...
            ${totalRow('GST (10%)', moneyCents(input.gst))}
...
export function buildInvoiceEmailText(input: InvoiceEmailInput): string {
  return [
    ...
    `Subtotal (ex GST): ${moneyCents(input.subtotalEx)}`,
...
export async function sendInvoiceEmail(input: InvoiceEmailInput): Promise<SendResult> {
  if (!isValidEmail(input.to)) return { ok: false, error: 'invalid_email' }
  const bcc = (process.env.PROPOSAL_BCC || DEFAULT_BCC).trim()
  const toLower = input.to.trim().toLowerCase()
  const cc = parseEmailList(input.cc).filter(e => e.toLowerCase() !== toLower)
